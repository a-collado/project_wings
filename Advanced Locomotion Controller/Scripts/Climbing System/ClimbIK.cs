...
namespace FasTPS
{
    public class ClimbIK : MonoBehaviour
    {
        ...
        public void UpdateAllTargetPositions(Point p)
        {
            ...
            if (rhHolder.target)
                rhTargetPosition = rhHolder.target.transform.position;

            IKPositions lfHolder = p.ReturnIK(AvatarIKGoal.LeftFoot);
            if (lfHolder.target)
                lfTargetPosition = lfHolder.target.transform.position;

            IKPositions rfHodler = p.ReturnIK(AvatarIKGoal.RightFoot);
            if (rfHodler.target)
                rfTargetPosition = rfHodler.target.transform.position;
        }
        ...
        void UpdateIK(AvatarIKGoal ik, IKPositions holder, Transform helper, float weight, AvatarIKHint h)
        {
            if(holder != null)
            {
